...
@onready var sprite: Node3D = $SubViewportContainer/SubViewport/Parasite_Asteroid
...
@onready var camera: Camera3D = $SubViewportContainer/SubViewport/Camera3D_parasite_Asteriod

# Asteroid properties
@export var damage: int = 2 : set = set_damage, get = get_damage
@export var movement_speed: int = 200 : set = set_movement_speed, get = get_movement_speed
@export var fire_debuff: int = 80   # percent fire rate reduction
@export var worth = 1
...
func _on_area_entered(area: Area2D) -> void:
	...
	# Turret collision
	if area.get_collision_layer_value(6): # turrets are on layer 6
		print("Parasite asteroid hit: ", area.name, " | parent: ", area.get_parent().name)
		var turret = area.get_parent()
		if turret and turret.has_method("apply_fire_rate_debuff"):
			turret.apply_fire_rate_debuff(fire_debuff)  # reduce fire rate
			print("Debuff applied to turret!")
		else:
			print("No debuff method found on: ", turret)
		queue_free()
